...
public class WithdrawForm extends JFrame implements ActionListener,FocusListener {
	...
	JButton btnWithdraw, btnClear,btnCancel;
	ButtonGroup bgType;
	String[] sList = { "Current", "Fixed"};
    String st,s;
	public WithdrawForm(){
		this.setTitle("Withdraw Form");
		...
		tfAccno = new JTextField(15);tfAccno.addFocusListener(this);
		...
		btnPanel=new JPanel();
		btnPanel.add(btnWithdraw); btnPanel.add(btnClear);btnPanel.add(btnCancel);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnWithdraw) {
			if(!(tfAccno.getText().isEmpty())){				
				/*String amt=JOptionPane.showInputDialog(this, "Please enter your withdraw amount");
	...
	void queryAccountInformation(){
		...
		try {
			...
			while(rs.next()){
				...
				if(jrbT1.getLabel().equals(rs.getString("AccType")))
					jrbT1.setSelected(true);
